...
export const SnakeGame = () => {
  ...
  const tick = useCallback(() => {
    setSnake((prev) => {
      ...
      if (head[0] < 0 || head[0] >= ROWS || head[1] < 0 || head[1] >= COLS) {
        setGameOver(true);
        setScore((s) => { addPoints(s); updateStreak("snake"); toast.info(`Game Over! +${s} points`); return s; });
        return prev;
      }
  ...
  const reset = () => {
    ...
    setSnake(initial); setFood(randomFood(initial)); dirRef.current = [0, 1]; setDir([0, 1]); setGameOver(false); setScore(0); setStarted(false);
  };
            ...
            {Array.from({ length: COLS }, (_, c) => {
              ...
              return (
                <div key={c} className={`w-4 h-4 rounded-sm transition-colors ${
                  ...
                  : isFood ? "bg-destructive shadow-[0_0_8px_hsl(var(--destructive)/0.5)]"
                  : "bg-background/20"
                }`} />
              );
            })}

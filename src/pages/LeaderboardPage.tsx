...
import { Trophy, ArrowLeft, Medal, Crown, User, Plus, AlertTriangle, Trash2 } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { getTotalWins, getTotalLosses, getPlayerName, setPlayerName, syncLeaderboard } from "@/lib/streaks";
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <input
                    ...
                    maxLength={20}
                    className={`w-full bg-background border rounded-lg px-3 py-2 text-foreground font-display text-sm focus:outline-none transition-colors ${nameError ? "border-destructive focus:border-destructive" : "border-border focus:border-primary"
                      }`}
                  />
                  {nameError && (
            ...
            {entries.map((entry, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${i === 0
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-display text-sm truncate ${i === 0 ? "text-accent" : isCurrentPlayer ? "text-primary" : "text-foreground"
                      }`}>
                      {entry.player_name}

...
const AuthPage = () => {
  ...
  useEffect(() => {
    if (user) {
      // Set player name from Google profile
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="animate-pulse font-display text-primary text-xl">Loading...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background relative flex items-center justify-center p-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground font-display tracking-wider text-sm">Sign in to track your scores</p>
        </div>
        ...
        <p className="text-center text-muted-foreground/50 text-xs mt-6">
          By signing in, you agree to have fun 🎮
        </p>
      </motion.div>
    </div>
  );

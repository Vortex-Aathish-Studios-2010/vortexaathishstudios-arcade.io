...
const EntertainmentGamePage = () => {
  ...
  const [matchMode, setMatchMode] = useState<"select" | "bot" | "friends" | null>("select");
  ...
  const renderFriendsSelection = () => (
    <div className="flex flex-col items-center gap-6 w-full max-w-md mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 w-full">
        <button 
          onClick={() => setMatchMode("bot")} // Near is same as bot/local for now
          className="flex flex-col items-center gap-4 p-8 bg-[hsl(var(--sport-card))] border border-[hsl(var(--sport-border))] rounded-2xl hover:border-[hsl(var(--sport-primary))] transition-all group"
        >
  ...
  return (
    ...
    >
      ...
      {/* Game Content */}
      <div className="flex-1 flex items-start justify-center p-4 pt-6 overflow-hidden">
        <motion.div
